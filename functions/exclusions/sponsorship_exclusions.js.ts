...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  // EDGE FILTERING: Filter rules based on server time BEFORE returning them to client
  ...
  if (activeRules.length === 0) return new Response(`/* AdExclusion: No active rules at this moment (${croTime}) */`, { headers });
